...
class SessionManager {
  ...
  session.error = null; session.lastError = null;
    ...
    try {
      ...
      const socket = makeWASocket({ version, auth: state, printQRInTerminal: false, logger: pino({ level: 'silent' }), getMessage: async () => ({}) });
      ...
      socket.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
          ...
          if (lastDisconnect?.error) {
            ...
            if (isLoggedOut) {
              // mark session error and remove session in-memory (but preserve folder unless delete requested)
              session.error = true; session.lastError = 'Device logged out by WhatsApp (loggedOut)';
              await this.removeSession(key, false);
