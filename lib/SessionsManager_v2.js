      ...
      for (const k of keys) {
        ...
        try {
          ...
          try {
            ...
            console.warn(
              `initSessions: failed to mark device ${k} as error:`,
              e && e.message
            );
          }
        }
                ...
                try {
                  if (session.deviceId) {
                    const resp = await this.pool.query(
                      "SELECT response FROM autoreply WHERE keyword = ? AND device_id = ? AND status = ? LIMIT 1",
                      [lowerMessage, session.deviceId, "active"]
                    );
                    rows = resp[0];
                  } else {
                    const resp = await this.pool.query(
                      "SELECT response FROM autoreply WHERE keyword = ? AND device_id = (SELECT id FROM devices WHERE device_key = ? LIMIT 1) AND status = ? LIMIT 1",
                      [lowerMessage, key, "active"]
                    );
                    rows = resp[0];
                  }
                } catch (e) {
