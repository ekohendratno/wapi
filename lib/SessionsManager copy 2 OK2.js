...
class SessionManager {
  ...
  async createSession(key) {
    ...
    try {
      ...
      const backupFile = path.join(sessionPath, "creds.backup.json");

      if (!fs.existsSync(credsFile) && fs.existsSync(backupFile)) {
        try {
          fs.copyFileSync(backupFile, credsFile);
          fs.chmodSync(credsFile, 0o600); // aman: hanya owner yang bisa baca/tulis
          console.log(`♻️ Restored creds.json from backup for session ${key}`);
        } catch (e) {
          console.error(`Failed to restore creds.json for ${key}:`, e.message);
        }
      }
      ...
      socket.ev.on("connection.update", async (update) => {
        ...
        if (qr && Date.now() - (session.lastQRUpdate || 0) > 30_000) {
          ...
          try {
            ...
            fs.chmodSync(path.join(sessionPath, "qr.png"), 0o644); 
