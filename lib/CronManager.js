...
class CronManager {
  ...
  async processMessages(session, type) {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      if (String(process.env.FEATURE_OPT_IN) === "1" && msgType !== "group") {
        const cleanRecip = number.replace(/\D/g, "");
        ...
        if (status === "blocked") {
          await this.pool.query(
            ...
            ["Penerima telah memblokir/unsubs notifikasi", currentTime, id]
          );
          continue;
        }

        if (status === "pending" && effectiveTag !== "opt-in") {
          await this.pool.query(
            "UPDATE messages SET status='failed', response=?, updated_at=? WHERE id=?",
            ["Penerima belum melakukan Opt-In (Persetujuan)", currentTime, id]
          );
          console.warn(
            `🚫 [Anti-Ban] Skip message ${id} to ${number} (No Opt-In)`
          );
          continue;
        }
      }
