...
class CronManager {
  ...
  async initCrons() {
    ...
    const t5 = cron.schedule(
      "*/5 * * * *",
      () => this.safeExec(this.checkPendingTransactions, "Pending transactions check"),
      { timezone: "Asia/Jakarta" }
    );
    this.tasks.push(t5);
  }
  ...
  async removeRemovedDeviceSessions() {
    ...
    for (const r of rows) {
      ...
      try {
        ...
        if (deviceId) {
          await connection.query("DELETE FROM messages WHERE device_id = ?", [deviceId]);
          await connection.query("DELETE FROM autoreply WHERE device_id = ?", [deviceId]);
        }

        // Groups use device_key
        await connection.query("DELETE FROM `groups` WHERE device_key = ?", [r.device_key]);

        // Finally delete device row
        await connection.query("DELETE FROM devices WHERE device_key = ?", [r.device_key]);
        ...
        console.warn(`Failed to fully remove device ${r.device_key}:`, err && err.message);
      } finally {
        connection.release();
      }
    }
    console.log(`🗑️ Removed ${count} sessions for 'deleted' devices (finalized user deletions).`);
  }
  ...
  async checkPendingTransactions() {
    ...
    const expiryMinutes = parseInt(process.env.DUITKU_EXPIRY_MINUTES || '10', 10);
    const now = moment().tz('Asia/Jakarta');
    const cutoff = now.clone().subtract(expiryMinutes, 'minutes').format('YYYY-MM-DD HH:mm:ss');

    console.log(`🔍 Checking pending transactions older than ${expiryMinutes} minutes (cutoff=${cutoff})`);
    ...
    try {
      ...
      for (const tx of rows) {
        ...
        try {
          ...
          console.log(`⏳ Marking expired pending transaction ${merchantOrderId} -> failed`);
          if (this.billingManager && typeof this.billingManager.updateTransactionStatus === 'function') {
            await this.billingManager.updateTransactionStatus(merchantOrderId, 'failed');
          } else {
            await this.pool.query(`UPDATE transactions SET status = 'failed', updated_at = CURRENT_TIMESTAMP WHERE merchantOrderId = ?`, [merchantOrderId]);
          }
        } catch (err) {
          console.error(`❌ Failed to update transaction ${merchantOrderId}:`, err && err.message);
        }
      }
    } catch (err) {
      console.error('❌ Error checking pending transactions:', err && err.message);
    }
  }
